...
export function CategoryForm({ initialData, billboards }: CategoryFormProps) {
  ...
  const description = initialData ? "Editar categoria" : "Criar categoria";
  const toastMessage = initialData
    ? "Categoria atualizada!"
    : "Categoria criada!";
  const action = initialData ? "Salvar mundanças" : "Criar";
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error(
        "Tenha certeza que você deletou todos os produtos usando essa categoria!"
      );
    } finally {
      setLoading(false);
      setOpen(false);
